...
bool HWSprite::CalculateVertices(HWDrawInfo* di, FVector3* v, DVector3* vp)
{
	...
	// DomeDoom: while rendering cubemap faces, billboards must face the camera
	// POSITION (view-direction-independent) so sprites line up across face
	// seams instead of splitting. Overrides all per-actor/particle preferences.
	if (gCubemapFaceRender)
		drawBillboardFacingCamera = true;
